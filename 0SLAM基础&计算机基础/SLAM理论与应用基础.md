...
假设这种线性关系为：![f(x)=ax+b](https://math.jianshu.com/math?formula=f(x)%3Dax%2Bb)

总误差的平方为：$e=\sum(f(x_i)-y_i)^2=\sum(ax_i+b-y_i)$​，根据多元微分的知识，当它们的偏微分等于0时，e可取最小值。
$$
\frac{\delta e}{\delta a}=2\sum(ax_i+b-y_i)x_i=0\\
\frac{\delta e}{\delta b}=2\sum(ax_i+b-y_i)=0\\
$$
整理得：
$$
\sum x_i^2*a+\sum x_i*b-\sum x_iy_i=0\\
\sum x_i*a+n*b-\sum y_i=0\\
$$
到这里可以直接根据构造Ax=b矩阵求逆求解如下，也可以直接消元求二元一次方程组如后面：

![image-20221005212720498](SLAM理论与应用基础.assets/image-20221005212720498.png)
...
$$
对于形如$y=ax^3+bx^2+cx+d$，方法一样求偏导，然后矩阵求逆。只有下面非线性函数的拟合才会用到迭代求解的方法。
