...
- 把候选关键帧及其共视组的所有地图点记为vvpMatchedMPs
- 通过DBoW2找到 和当前关键帧匹配的地图点
...
| **类目**     | ORB                                                          | **ORB-SLAM2**                                                | **ORB-SLAM3**                                                |
| :----------- | ------------------------------------------------------------ | :----------------------------------------------------------- | :----------------------------------------------------------- |
| 传感器       | 单目                                                         | 单目、双目、RGB-D                                            | 单目、单目+IMU、双目、双目+IMU、RGB-D                        |
| 相机模型     | 针孔模型                                                     | 针孔模型。双目模式下假设双目完成了立体矫正，匹配点位于水平的极线附近。 | 抽象模型，支持针孔、鱼眼模型。双目模式下不依赖于立体矫正，而是将双目看作两个相对位置不变的独立的、具有重叠视角的单目相机。 |
| 地图         | 单地图                                                       | 单地图                                                       | 多地图，支持地图融合                                         |
| 地图初始化   | 初始化较慢，且没有尺度信息                                   | 单目模式下地图初始化较慢，且没有尺度信息                     | 单目+IMU模式下可以较快的初始化地图，且具有绝对尺度           |
...
| 特点         | 宽基线匹配；大场景实时；统一的ORB特征；地图点和关键帧的选取策略（适者生存）；初始化场景解耦； | 多传感器支持；多场景精度和鲁棒性超越同类方法；远近点；       | 多模型支持；IMU加持；VIO完全MAP；多地图提高召回率（多地图，回环策略）；短、中、长期数据关联； |
...
| 初始化的异同   | ORB-SLAM3                                                    | VINS-Mono                                                    |
| -------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 成功准则       | 加速度大于重力的0.5%（论文有代码无）                         | 重力误差在10%以内，IMU测量值有一定的变化范围                 |
| 加速度计bias   | 同时估计加速度计和陀螺仪的偏置                               | 不估计加速计的偏置，认为加速计的bias和g的比值很小，难以估计出 |
| 陀螺仪bias     | 陀螺仪偏置放在目标函数中一起估计                             | 陀螺仪偏置单独拿出来估计                                     |
| 尺度恢复策略   | 第一次初始值设为1，初始化100s内会不断优化这个值              | 只在初始化的时候恢复该尺度                                   |
| 姿态参数化     | 基于SO(3)                                                    | 基于四元数                                                   |
| 惯性系对齐方式 | 优化过程中同时优化惯性系到第一帧的旋转矩阵                   | 优化得到重力在第一帧的测量值，然后对重力重新参数化，以限制其模长，最后重新来一次优化 |
| 求解过程       | 三次非线性优化（Local mapping，Inertial only optimization，Full BA） | 三次非线性优化（sfm，求陀螺仪偏置，Full BA），两次线性求解（求尺度、速度初值）和相机-IMU外参。 |
